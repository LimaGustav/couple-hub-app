...
export function HomePage({ balance, tasks, memories }: HomePageProps) {
  ...
  const metrics = [
    { label: "Love Tokens", value: balance, Icon: Coins, accentClass: "bg-primary", bgClass: "bg-[var(--secondary)]" },
    { label: "Aprovadas", value: approved, Icon: Check, accentClass: "bg-[#16a34a]", bgClass: "bg-[#f0fdf4]" },
    { label: "Pendentes", value: pending, Icon: Zap, accentClass: "bg-[#d97706]", bgClass: "bg-[#fffbeb]" },
  ];
  ...
  return (
    <div className="p-5 md:p-8 max-w-4xl mx-auto">
      <div className="mb-6 md:mb-8">
        ...
        <p className="text-sm text-muted-foreground">Sábado, 21 de Junho de 2026</p>
      </div>
      ...
      <div className="md:hidden flex items-center gap-3 p-4 rounded-2xl mb-5 border border-border bg-muted text-muted-foreground">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">{couple.days} dias juntos ❤️</p>
        </div>
      </div>
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="mt-4 rounded-2xl overflow-hidden bg-card border border-border">
          <div className="relative h-44 md:h-52 bg-muted">
            <img src={memories[0].image} alt={memories[0].text} className="w-full h-full object-cover" />
            <div className="absolute inset-0 gradient-fade-top" />
            <div className="absolute bottom-0 left-0 p-4 text-white">
              <p className="text-sm font-medium leading-snug">{memories[0].text}</p>
              <div className="flex items-center gap-1.5 mt-1 text-xs opacity-75">
                <MapPin className="w-3 h-3" />
                <span>
                  {memories[0].location} · {memories[0].date}
                </span>
              </div>
            </div>
          </div>
